document.addEventListener('DOMContentLoaded', () => {
    ...
    const setSubtitle = num => {
        let str = ''
        let lastNum = num.toString()[num.toString().length - 1]
        if (lastNum == 1) srt = ''
        else if (lastNum >= 2 && lastNum <= 4) str = 'a'
        else if (lastNum >= 5 && lastNum <= 9 || lastNum == 0) str = 'ов'
        document.querySelector('.gph__header-subtitle-num').innerHTML = num +  `<span> договор${str} в работе</span>`
    }
    ...
    function setModalBody(item) {
        ...
        <stop stop-color="${item.expired <= 0 ? '#2479FD' : '#EF6B74'}"/>
