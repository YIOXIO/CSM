const tabs = document.querySelectorAll('.enrollment__tab')
const tabcontent = document.querySelectorAll('.enrollment__tabcontent')
...
tabs.forEach((tab, index) => {
 tab.addEventListener('click', () => {
  ...
  tabs.forEach(item => {
   ...
   item.classList.remove('enrollment__tab_active')
  })

  tab.querySelector('svg').querySelector('path').setAttribute('stroke', '#217AFF')
  tab.classList.add('enrollment__tab_active')
