document.addEventListener('DOMContentLoaded', () => {
    ...
    const setTitle = title => document.querySelector('.dpo__header-title').textContent = title

    // ф-ия для установки в подзаголовок кол-ва элементов в отображемом списке.
    const setSubtitle = num => document.querySelector('.dpo__header-subtitle span').textContent = num
    ...
    const setProgressbar = (id, fact, isIcrease, modifier = 'list') => {
        ...
        return `
            <svg class="dpo__progressbar dpo__progressbar_${modifier}" viewBox="0 0 1054 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect opacity="0.05" x="-2" width="100%" height="100%" fill="white"/>
                <rect y="1" width="${fact <= 100 ? fact : 100}%" height="100%" fill="url(#${id})"/>
    ...
    const menuItem = item => {
        ...
        return `
            <div class="dpo__menu-block" data-id=${id}>
                <svg class="dpo__graphiс" width="100%" overflow="visible" viewBox="0 0 120 3" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect class="dpo__graphiс-row" height="5%"  width="10%" fill="${color}" x="3" y="0.4" />
                    <rect  class="dpo__graphiс-right-rect" stroke="${color}" x="10%" y="0"  fill="white"/>
                    <rect  class="dpo__graphiс-left-rect" stroke="${color}" x="0" y="0" fill="transparent"/>
                </svg>
                <div class="dpo__menu-block__wrap">
                    <div class="dpo__menu-block__descr">${name}</div>
                </div>
                <div class="dpo__menu-block__info">
                    ${setProgressbar(id, fact, isIcrease, 'menu')}

                    <div class="dpo__menu-block__indicators">
                        <div class="dpo__menu-block__value">${fact}%</div>

                        <div class="dpo__menu-block__current-plan">
                            <span class="dpo__menu-block__current-plan-value">${plan}</span>
                            <span class="dpo__menu-block__current-plan-descr">тыс.руб.</span>
                        </div>
                    </div>
                    
                    <img class="dpo__menu-block__alarm" data-id="${id}" src="../assets/img/plan-icons/alarm.svg">
                </div> 
    ...
    function setMenu(data) {
        ...
        let out = document.querySelector('.dpo__tabcontent-block__wrap_menu')
        let menuResult = ''
    ...
    const menuBlocks = document.querySelectorAll('.dpo__menu-block')
    ...
    menuBlocks.forEach(elem => {
        elem.addEventListener('click', function (e) {
            ...
            if (e.target.className == 'dpo__menu-block__alarm') {
                overlay.classList.add('overlay_show')
                modalLarge.classList.add('modal_show')
                if (id !== undefined) {
                    dpoData.find(partner => {
            ...
            // Логика связанная с непосредственным выбором элемента меню.
                ...
                // Рендер контента.
                setContent(getSortData(dpoData, id))

                // Отображение диаграммы. 
                menuBlocks.forEach(b => b.classList.remove('dpo__menu-block_opened'))
                menuDiagramBoxes.forEach(b => b.innerHTML = '')
                elem.classList.add('dpo__menu-block_opened')
                showChart(elem, id, dpoData, 'menu')
            }
    ...
    // Сброс фильтра.
    document.querySelector('.header__link_get-all-dpo').addEventListener('click', () => {
        setContent(getSortData(dpoData))
        setTitle(defaultTitle)
        setLinesDefault()
        menuBlocks.forEach(b => b.classList.remove('dpo__menu-block_opened'))
        menuDiagramBoxes.forEach(b => b.innerHTML = '')
    ...
    class Partner {
        constructor(array, modifier) {
            this.array = array,
                this.className = 'dpo__tabcontent-block__wrap',
                this.modifier = modifier,
                this.parent = document.querySelector(`.dpo__tabcontent-block_${this.modifier}`),
        ...
        bg(index) {
            ...
            return `
                <svg class="dpo__tabcontent-block__bg dpo__tabcontent-block__bg_${bgOffsetModifier} hide" viewBox="0 0 1269 675" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        divider() {
            return `
                <svg class="dpo__tabcontent-block__divider" viewBox="0 0 800 39" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        createBlockItem(item, index) {
            ...
            return `
                <div class="dpo__tabcontent-block__item" data-id="${item.id}">
                
                    ${index === 3 ? '<div class="diagram-box_list"></div>' : ''} 

                    ${this.bg(index)}
                    
                    <div class="dpo__tabcontent-block__info">
                        <svg class="dpo__tabcontent-block__mark" width="100%" overflow="visible" viewBox="0 0 120 3" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect stroke="${item.color}" x="0" y="0" fill="white"/>
                        </svg>
                        <div class="dpo__tabcontent-block__descr">${item.name}</div>
                        <div class="dpo__tabcontent-block__value">${item.fact}%</div>
                    </div>
                    ${setProgressbar(this.modifier + '-' + index, Math.ceil(item.fact), item.isIncrease)}
         
                    <div class="dpo__tabcontent-block__info dpo__tabcontent-block__info_bottom">

                        ${this.divider()}
                       
                        <div class="dpo__tabcontent-block__current-plan">
                            <span class="dpo__tabcontent-block__current-plan-value">${item.plan}</span>
                            <span class="dpo__tabcontent-block__current-plan-descr">тыс.руб.</span>
                        </div>
                    </div>
        ...
        render() {
            ...
            if (this.array.length > 3) {
                ...
                element.classList.add(this.className)
                element.classList.add('dpo__tabcontent-block__wrap_min-content')
            }
            let str = ''
    ...
    function createPages(modifier) {
        ...
        tabContent.classList.add('dpo__tabcontent', `dpo__tabcontent_p${modifier}`)

        document.querySelector('.dpo__tabcontainer').append(tabContent)
    }
    ...
    const setPage = pageNumber => {
        return `
            <div class="dpo__tabcontent-wrap">
                <div class="dpo__tabcontent-element">
                    <div class="dpo__tabcontent-block dpo__tabcontent-block_p${pageNumber}-m">
                            <svg class="dpo__tabcontent-block-bg" viewBox="0 0 1269 1703" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="dpo__tabcontent-block__wrap dpo__tabcontent-block__wrap_p${pageNumber}-m"></div>
                    </div>		
                </div>

                <div class="dpo__tabcontent-element">
                    <div class="dpo__tabcontent-block dpo__tabcontent-block_p${pageNumber}-r">
                        <svg class="dpo__tabcontent-block-bg" viewBox="0 0 1269 1703" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1.00024 1631.95L1.0001 99.5907L1.0001 36.1923L38.5906 0.999875L248.186 0.999856L267.551 19.1292L315.394 19.1292L329.063 5.2656L414.496 5.26559L440.695 30.8599L1003.41 30.8599L1029.61 5.26554L1233.83 5.26552L1268 38.325L1268 1641.55L1225.85 1681L565.997 1681L544.354 1701.27L113.772 1701.27L92.1288 1681L53.3992 1681L1.00024 1631.95Z" fill="#112054" fill-opacity="0.68" stroke="#217AFF" stroke-width="2"/>
                        </svg>

                        <div class="dpo__tabcontent-block__wrap dpo__tabcontent-block__wrap_p${pageNumber}-r"></div>
                    </div>	
                </div>
            </div>
        `
    ...
    const btn = num => {
        return `
            <div class="dpo__tabitem dpo__tabitem-btn ${num === 1 ? 'dpo__tabitem_active' : null}" data-page=${num}>
                <svg viewBox="0 0 110 110" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M80.9858 109L109 80.9858V1H22.4142L1 22.4142V109H80.9858Z"/>	
                </svg>
                <div class="dpo__tabitem-value">${num}</div>	
            </div>
        `
    }

    const next = `
        <div class="dpo__tabitem dpo__tabitem-prev">
            <svg viewBox="0 0 110 110" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M80.9858 109L109 80.9858V1H22.4142L1 22.4142V109H80.9858Z"/>	
            </svg>
            <div class="dpo__tabitem-value">&lt;</div>	
        </div>
    `

    const prev = `
        <div class="dpo__tabitem dpo__tabitem-next">
            <svg viewBox="0 0 110 110" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    function setContent(data) {
        ...
        const tabsContent = document.querySelectorAll('.dpo__tabcontent')
        ...
        function innerPaginationToPage(pagination) {
            document.querySelector('.dpo__tabheader-btns').innerHTML = ''
            let result = []
            if (pages == 1) {
                return
            }
            ...
            document.querySelector('.dpo__tabitem-next').addEventListener('click', nextClick)
            document.querySelector('.dpo__tabitem-prev').addEventListener('click', prevClick)
            togglePage()
        }
        ...
        function nextClick() {
            ...
            if (currentPage >= pagination.length) {
                ...
                document.querySelectorAll('.dpo__tabitem-btn').forEach(i => {
                    i.classList.remove('dpo__tabitem_active')
                    if (i.getAttribute('data-page') == currentPage) {
                        i.classList.add('dpo__tabitem_active')
                    }
                })
        ...
        function togglePage() {
            document.querySelectorAll('.dpo__tabitem-btn').forEach(i => {
                i.addEventListener('click', (e) => {
                    document.querySelectorAll('.dpo__tabitem-btn').forEach(item => {
                        item.classList.remove('dpo__tabitem_active')
                    })
        ...
        document.querySelector('.dpo__tabheader').addEventListener('selectstart', () => {
            return false
        })
        ...
        let listItems = document.querySelectorAll('.dpo__tabcontent-block__item')
        let listDiagramBoxes = document.querySelectorAll('.diagram-box_list')
        let listItemBgs = document.querySelectorAll('.dpo__tabcontent-block__bg')

        // Отображение диаграммы.
        listItems.forEach(elem => {
            elem.addEventListener('click', () => {
                let id = elem.getAttribute('data-id')
                let bg = elem.querySelector('.dpo__tabcontent-block__bg')
        ...
        document.addEventListener('click', (e) => {
            ...
            if (elem == 'dpo__wrapper' || elem == 'dpo__tabcontent-wrap' || elem == 'dpo__tabcontent-block__wrap dpo__tabcontent-block__wrap_min-content' || elem == 'dpo__header-title' || elem == 'dpo__header' || elem == 'subtitle' || elem == 'dpo__tabcontent dpo__tabcontent_p1') {
                listItemBgs.forEach(bg => bg.classList.remove('show'))
                listDiagramBoxes.forEach(b => b.innerHTML = '')
            }
        })
